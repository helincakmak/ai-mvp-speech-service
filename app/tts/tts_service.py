...
class TTSService:
    ...
    def text_to_speech(
        ...
        try:
            ...
            # Fade-in/out uygula (robotik sesi önler)
            ...
            logger.info(f"✅ TTS başarılı! Ses boyutu: {len(audio_bytes)} bytes, Süre: {audio_duration:.2f}s")
    ...
    def _apply_fade(self, audio: np.ndarray) -> np.ndarray:
        """Başına ve sonuna fade-in/out uygula (sert giriş/çıkışı yumuşatır)"""
    ...
    def _numpy_to_wav(self, samples: np.ndarray, sample_rate: int) -> bytes:
        """NumPy array'i WAV bytes'a çevir"""
        # Float32'den Int16'ya çevir
        audio_int16 = (samples * 32767).astype(np.int16)
        
        # WAV dosyası oluştur (memory'de)
        wav_io = io.BytesIO()
